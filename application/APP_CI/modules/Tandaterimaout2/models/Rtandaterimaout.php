...
class Rtandaterimaout extends CI_Model {
        ...
        function getGrid($data){
            ...
            $this->db->select(' tr_receipt_detail_out.*
            ...
            $this->db->from('tr_receipt_detail_out');
        ...
        function reportPreview($id){
            ...
            $this->db->select(' tr_receipt_out.*, m_customer.*

                            ',FALSE);

            $this->db->from('tr_receipt_out');

            $this->db->join('m_customer', 'm_customer.customer_id = tr_receipt_out.receipt_to', 'INNER');
        ...
        function load_default($start,$limit,$filter){
            ...
            $this->db->select(' SQL_CALC_FOUND_ROWS tr_receipt_out.*, m_customer.*
            ...
            $this->db->where("tr_receipt_out.receipt_id <>", 0);

            $this->db->like('tr_receipt_out.receipt_doc',$dtfilter[0]['value']);   

            $this->db->limit($limit,$start);

            $this->db->order_by("tr_receipt_out.receipt_id","DESC");
